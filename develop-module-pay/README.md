...
支付模块，提供支付应用、渠道、订单、退款、回调通知、钱包等支付基础能力。
...
| Maven Artifact | `develop-module-pay` |
| Packaging | `pom` |
| Java 源文件数量 | 283 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-pay-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-pay-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-pay -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-pay -am -Dmaven.test.skip=true
```
