...
仓储管理模块，提供仓库、库区、库位、库存、入库、出库、移库、盘点等 WMS 能力。
...
| Maven Artifact | `develop-module-wms` |
| Packaging | `pom` |
| Java 源文件数量 | 166 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-wms-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-wms-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-wms -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-wms -am -Dmaven.test.skip=true
```
