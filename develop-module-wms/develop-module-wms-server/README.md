...
wms 模块下，仓库管理系统（Warehouse Management System）。 例如说：仓库、物料、库存、入库、出库、移库、盘库等等

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-wms/develop-module-wms-server` |
| Maven Artifact | `develop-module-wms-server` |
| Packaging | `jar` |
| Java 源文件数量 | 157 |
| 模块说明 | wms 模块下，仓库管理系统（Warehouse Management System）。 例如说：仓库、物料、库存、入库、出库、移库、盘库等等 |
...
| `develop-module-wms-api` | 业务模块 API / server / core 依赖。 |
...
# 运行该模块测试
mvn test -pl develop-module-wms/develop-module-wms-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-wms/develop-module-wms-server -am -Dmaven.test.skip=true
```
