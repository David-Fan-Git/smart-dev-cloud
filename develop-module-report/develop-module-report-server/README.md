# develop-module-report-server

## 模块定位

report 模块，主要实现数据可视化报表等功能： 1. 基于「积木报表」实现，打印设计、报表设计、图形设计、大屏设计等。

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-report/develop-module-report-server` |
| Maven Artifact | `develop-module-report-server` |
| Packaging | `jar` |
| Java 源文件数量 | 44 |
| 模块说明 | report 模块，主要实现数据可视化报表等功能： 1. 基于「积木报表」实现，打印设计、报表设计、图形设计、大屏设计等。 |
...
| `develop-module-report-api` | 业务模块 API / server / core 依赖。 |
...
```bash
# 编译该模块及其依赖
mvn compile -pl develop-module-report/develop-module-report-server -am

# 运行该模块测试
mvn test -pl develop-module-report/develop-module-report-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-report/develop-module-report-server -am -Dmaven.test.skip=true
```
