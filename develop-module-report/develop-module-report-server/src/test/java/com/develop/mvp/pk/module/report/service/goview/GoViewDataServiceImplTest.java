package com.develop.mvp.pk.module.report.service.goview;
...
public class GoViewDataServiceImplTest extends BaseDbUnitTest {
    ...
    public void testGetDataBySQL() {
        ...
        when(sqlRowSet.getObject("name")).thenReturn("David").thenReturn("David");
        ...
        assertEquals("David", dataBySQL.getSource().get(0).get("name"));
        assertEquals(2, dataBySQL.getSource().get(1).size());
        assertEquals(2L, dataBySQL.getSource().get(1).get("id"));
        assertEquals("David", dataBySQL.getSource().get(1).get("name"));
    }
