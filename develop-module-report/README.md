...
报表模块，集成 JimuReport / JimuBI，提供报表设计、数据可视化与 BI 能力。
...
| Maven Artifact | `develop-module-report` |
| Packaging | `pom` |
| Java 源文件数量 | 46 |
| 模块说明 | report 模块，主要实现数据可视化报表等功能。 |

## 子模块结构

| 子模块 | 职责 |
|---|---|
| `develop-module-report-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-report-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-report -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-report -am -Dmaven.test.skip=true
```
