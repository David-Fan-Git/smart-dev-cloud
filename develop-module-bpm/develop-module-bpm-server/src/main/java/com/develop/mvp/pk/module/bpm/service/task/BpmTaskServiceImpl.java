...
import com.develop.mvp.pk.framework.common.util.number.NumberUtils;
...
import com.develop.mvp.pk.module.bpm.controller.admin.task.vo.task.*;
import com.develop.mvp.pk.module.bpm.convert.task.BpmTaskConvert;
...
import com.develop.mvp.pk.module.bpm.enums.definition.*;
import com.develop.mvp.pk.module.bpm.enums.task.BpmCommentTypeEnum;
import com.develop.mvp.pk.module.bpm.enums.task.BpmReasonEnum;
import com.develop.mvp.pk.module.bpm.enums.task.BpmTaskSignTypeEnum;
...
import com.develop.mvp.pk.module.bpm.service.message.dto.BpmMessageSendWhenTaskTimeoutReqDTO;
...
import static com.develop.mvp.pk.module.bpm.framework.flowable.core.util.BpmnModelUtils.*;
...
//        }
        ...
        // TODO @David：https://t.zsxq.com/MNzqp 【flowable bug】：taskCreatedAfter、taskCreatedBefore 拼接的是 OR
        if (ArrayUtil.isNotEmpty(pageVO.getCreateTime())) {
...
//        }
    ...
    public void rejectTask(Long userId, @Valid BpmTaskRejectReqVO reqVO) {
        ...
        // 疑问：为什么要标记未通过呢？
        // 回答：例如说 A 任务被向前加签除 B 任务时，B 任务被审批不通过，此时 A 会被取消。而 develop-ui-admin-vue3 不展示“已取消”的任务，导致展示不出审批不通过的细节。
    ...
    public void delegateTask(Long userId, BpmTaskDelegateReqVO reqVO) {
        ...
        // 特殊：如果已经被委派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/develop-cloud/issues/ICJ153
    ...
    public void transferTask(Long userId, BpmTaskTransferReqVO reqVO) {
        ...
        // 3.1 设置任务所有人 (owner) 为原任务的处理人 (assignee)
        // 特殊：如果已经被转派（owner 非空），则不需要更新 owner：https://gitee.com/zhijiantianya/develop-cloud/issues/ICJ153
        if (StrUtil.isEmpty(task.getOwner())) {
            taskService.setOwner(taskId, task.getAssignee());
        }
    ...
    public void moveTaskToEnd(String processInstanceId, String reason) {
        ...
        // TODO David：目前发现并行分支情况下，会存在这个情况，后续看看有没更好的方案；
    ...
    public void withdrawTask(Long userId, String taskId) {
        ...
        // TODO @David：是否选择升级flowable版本解决taskCreatedAfter、taskCreatedBefore问题，升级7.1.0可以；包括 todo 和 done 那边的查询哇？？？ 是的！
    ...
    public void processTaskCreated(Task task) {
        ...
        TransactionSynchronizationManager.registerSynchronization(new TransactionSynchronization() {
             ...
             * 参见 <a href="https://gitee.com/zhijiantianya/develop-cloud/issues/IB7V7Q">issue</a> 反馈
             */
            @Override
