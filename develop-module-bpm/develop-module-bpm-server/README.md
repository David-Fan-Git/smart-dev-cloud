...
bpm 包下，业务流程管理（Business Process Management），我们放工作流的功能，基于 Flowable 6 版本实现。 例如说：流程定义、表单配置、审核中心（我的申请、我的待办、我的已办）等等

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-bpm/develop-module-bpm-server` |
| Maven Artifact | `develop-module-bpm-server` |
| Packaging | `jar` |
| Java 源文件数量 | 257 |
| 模块说明 | bpm 包下，业务流程管理（Business Process Management），我们放工作流的功能，基于 Flowable 6 版本实现。 例如说：流程定义、表单配置、审核中心（我的申请、我的待办、我的已办）等等 |
...
| `develop-module-bpm-api` | 业务模块 API / server / core 依赖。 |
...
# 运行该模块测试
mvn test -pl develop-module-bpm/develop-module-bpm-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-bpm/develop-module-bpm-server -am -Dmaven.test.skip=true
```
