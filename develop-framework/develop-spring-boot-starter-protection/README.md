...
| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-framework/develop-spring-boot-starter-protection` |
| Maven Artifact | `develop-spring-boot-starter-protection` |
| Packaging | `jar` |
| Java 源文件数量 | 29 |
| 模块说明 | 服务保证，提供分布式锁、幂等、限流、熔断等等功能 |
...
```bash
# 编译该模块及其依赖
mvn compile -pl develop-framework/develop-spring-boot-starter-protection -am

# 运行该模块测试
mvn test -pl develop-framework/develop-spring-boot-starter-protection

# 打包该模块及其依赖
mvn clean package -pl develop-framework/develop-spring-boot-starter-protection -am -Dmaven.test.skip=true
```
