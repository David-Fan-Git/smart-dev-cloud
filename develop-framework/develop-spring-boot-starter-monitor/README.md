...
| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-framework/develop-spring-boot-starter-monitor` |
| Maven Artifact | `develop-spring-boot-starter-monitor` |
| Packaging | `jar` |
| Java 源文件数量 | 8 |
| 模块说明 | 服务监控，提供链路追踪、日志服务、指标收集等等功能 |
...
```bash
# 编译该模块及其依赖
mvn compile -pl develop-framework/develop-spring-boot-starter-monitor -am

# 运行该模块测试
mvn test -pl develop-framework/develop-spring-boot-starter-monitor

# 打包该模块及其依赖
mvn clean package -pl develop-framework/develop-spring-boot-starter-monitor -am -Dmaven.test.skip=true
```
