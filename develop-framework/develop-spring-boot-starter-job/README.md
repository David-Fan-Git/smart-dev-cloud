...
| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-framework/develop-spring-boot-starter-job` |
| Maven Artifact | `develop-spring-boot-starter-job` |
| Packaging | `jar` |
| Java 源文件数量 | 4 |
| 模块说明 | 任务拓展，基于 XXL-Job 实现 |
...
```bash
# 编译该模块及其依赖
mvn compile -pl develop-framework/develop-spring-boot-starter-job -am

# 运行该模块测试
mvn test -pl develop-framework/develop-spring-boot-starter-job

# 打包该模块及其依赖
mvn clean package -pl develop-framework/develop-spring-boot-starter-job -am -Dmaven.test.skip=true
```
