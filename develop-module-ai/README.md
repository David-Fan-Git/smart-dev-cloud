...
AI 大模型模块，提供模型配置、聊天、图片、音乐、知识库、向量分段、工作流、工具调用等 AI 能力。
...
| Maven Artifact | `develop-module-ai` |
| Packaging | `pom` |
| Java 源文件数量 | 204 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-ai-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-ai-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-ai -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-ai -am -Dmaven.test.skip=true
```
