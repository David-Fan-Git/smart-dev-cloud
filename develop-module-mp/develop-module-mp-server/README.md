# develop-module-mp-server

## 模块定位

mp 模块，我们放微信微信公众号。 例如说：提供微信公众号的账号、菜单、粉丝、标签、消息、自动回复、素材、模板通知、运营数据等功能

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-mp/develop-module-mp-server` |
| Maven Artifact | `develop-module-mp-server` |
| Packaging | `jar` |
| Java 源文件数量 | 135 |
| 模块说明 | mp 模块，我们放微信微信公众号。 例如说：提供微信公众号的账号、菜单、粉丝、标签、消息、自动回复、素材、模板通知、运营数据等功能 |
...
| `develop-module-mp-api` | 业务模块 API / server / core 依赖。 |
...
```bash
# 编译该模块及其依赖
mvn compile -pl develop-module-mp/develop-module-mp-server -am

# 运行该模块测试
mvn test -pl develop-module-mp/develop-module-mp-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-mp/develop-module-mp-server -am -Dmaven.test.skip=true
```
