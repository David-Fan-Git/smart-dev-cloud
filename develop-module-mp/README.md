...
微信公众号模块，提供公众号账号、菜单、粉丝、标签、素材、消息、自动回复、模板通知等能力。
...
| Maven Artifact | `develop-module-mp` |
| Packaging | `pom` |
| Java 源文件数量 | 140 |
| 模块说明 | wechat 模块，主要实现微信平台的相关业务。 例如：微信公众号、企业微信 SCRM 等 |

## 子模块结构

| 子模块 | 职责 |
|---|---|
| `develop-module-mp-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-mp-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-mp -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-mp -am -Dmaven.test.skip=true
```
