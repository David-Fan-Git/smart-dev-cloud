...
crm 包下，客户关系管理（Customer Relationship Management）。 例如说：客户、联系人、商机、合同、回款等等

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-crm/develop-module-crm-server` |
| Maven Artifact | `develop-module-crm-server` |
| Packaging | `jar` |
| Java 源文件数量 | 255 |
| 模块说明 | crm 包下，客户关系管理（Customer Relationship Management）。 例如说：客户、联系人、商机、合同、回款等等 |
...
| `develop-module-crm-api` | 业务模块 API / server / core 依赖。 |
...
# 运行该模块测试
mvn test -pl develop-module-crm/develop-module-crm-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-crm/develop-module-crm-server -am -Dmaven.test.skip=true
```
