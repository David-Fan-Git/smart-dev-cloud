...
客户关系管理模块，管理客户、联系人、商机、合同、回款、线索、跟进记录等 CRM 能力。
...
| Maven Artifact | `develop-module-crm` |
| Packaging | `pom` |
| Java 源文件数量 | 269 |
| 模块说明 | crm 包下，客户关系管理（Customer Relationship Management）。 例如说：客户、联系人、商机、合同、回款等等 商业智能 BI 模块，包括：报表、图表、数据大屏等等 |

## 子模块结构

| 子模块 | 职责 |
|---|---|
| `develop-module-crm-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-crm-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-crm -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-crm -am -Dmaven.test.skip=true
```
