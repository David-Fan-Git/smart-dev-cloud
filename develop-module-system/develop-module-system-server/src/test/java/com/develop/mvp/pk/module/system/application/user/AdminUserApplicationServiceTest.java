...
import static com.develop.mvp.pk.module.system.application.user.service.AdminUserApplicationService.USER_INIT_PASSWORD_KEY;
...
@Import({AdminUserApplicationService.class, UserApplicationService.class, UserRepositoryImpl.class, UserUniquenessCheckerImpl.class})
public class AdminUserApplicationServiceTest extends BaseDbUnitTest {
    ...
    public void before() {
        // mock 初始化密码
        when(configApi.getConfigValueByKey(USER_INIT_PASSWORD_KEY)).thenReturn(success("developyuanma"));
    }
    ...
    public void testCreatUser_success() {
        ...
        when(deptService.getPostList(eq(reqVO.getPostIds()), isNull())).thenReturn(posts);
        // mock passwordEncoder 的方法
        when(passwordEncoder.encode(eq(reqVO.getPassword()))).thenReturn("developyuanma");
        ...
        assertEquals("developyuanma", user.getPassword());
    ...
    public void testImportUserList_02() {
        ...
        when(deptService.getDept(eq(dept.getId()))).thenReturn(dept);
        // mock passwordEncoder 的方法
        when(passwordEncoder.encode(eq("developyuanma"))).thenReturn("java");
