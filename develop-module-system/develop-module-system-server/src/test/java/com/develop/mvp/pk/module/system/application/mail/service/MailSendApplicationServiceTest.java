...
public class MailSendApplicationServiceTest extends BaseMockitoUnitTest {
...
//                .setFrom("奥特曼 <ydym_test@163.com>")
    ...
    public void testSendSingleMail_success() {
        ...
        doReturn(template).when(mailSendService).getMailTemplateByCodeFromCache(eq(templateCode));
        String title = RandomUtils.randomString();
        doReturn(title).when(mailSendService).formatMailTemplateContent(eq(template.getTitle()), eq(templateParams));
        String content = RandomUtils.randomString();
        doReturn(content).when(mailSendService).formatMailTemplateContent(eq(template.getContent()), eq(templateParams));
        // mock 邮箱账号方法
        MailAccountDO account = randomPojo(MailAccountDO.class);
        doReturn(account).when(mailSendService).getMailAccountFromCache(eq(template.getAccountId()));
        // mock 邮件日志方法
        Long mailLogId = randomLongId();
        doReturn(mailLogId).when(mailSendService).createMailLog(eq(userId), eq(UserTypeEnum.ADMIN.getValue()),
                ...
                eq(account), eq(template), eq(content), eq(templateParams), eq(true));
    ...
    public void testSendSingleMail_successWhenMailTemplateEnable() {
        ...
        // mock 邮件日志方法
        Long mailLogId = randomLongId();
        doReturn(mailLogId).when(mailSendService).createMailLog(eq(userId), eq(userType),
    ...
    public void testSendSingleMail_successWhenMailTemplateDisable() {
        ...
        doReturn(mailLogId).when(mailSendService).createMailLog(eq(userId), eq(userType),
                ...
                argThat(Collection::isEmpty),
                argThat(Collection::isEmpty),
                eq(account), eq(template), eq(content), eq(templateParams), eq(false));
    ...
    public void testValidateMailTemplateValid_notExists() {
        // 准备参数
        String templateCode = RandomUtils.randomString();
        doReturn(null).when(mailSendService).getMailTemplateByCodeFromCache(eq(templateCode));
    ...
    public void testDoSendMail_success() {
        try (final MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            // 准备参数
            MailSendMessage message = randomPojo(MailSendMessage.class, o -> o.setNickname("David"));
            // mock 方法（获得邮箱账号）
            MailAccountDO account = randomPojo(MailAccountDO.class, o -> o.setMail("7685@qq.com"));
            doReturn(account).when(mailSendService).getMailAccountFromCache(eq(message.getAccountId()));

            // mock 方法（发送邮件）
            String messageId = randomString();
            mailUtilMock.when(() -> MailUtil.send(
                    argThat(mailAccount -> {
                        assertEquals("David <7685@qq.com>", mailAccount.getFrom());
            ...
            doNothing().when(mailSendService).updateMailSendResult(eq(message.getLogId()), eq(messageId), isNull());

            // 调用
            mailSendService.doSendMail(message);
            // 断言
            verify(mailSendService).updateMailSendResult(eq(message.getLogId()), eq(messageId), isNull());
        }
    }
    ...
    public void testDoSendMail_exception() {
        try (MockedStatic<MailUtil> mailUtilMock = mockStatic(MailUtil.class)) {
            ...
            doNothing().when(mailSendService).updateMailSendResult(eq(message.getLogId()), isNull(), same(e));

            // 调用
            mailSendService.doSendMail(message);
            // 断言
            verify(mailSendService).updateMailSendResult(eq(message.getLogId()), isNull(), same(e));
        }
    }
