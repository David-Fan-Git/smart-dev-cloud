...
| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-iot/develop-module-iot-server` |
| Maven Artifact | `develop-module-iot-server` |
| Packaging | `jar` |
| Java 源文件数量 | 252 |
| 模块说明 | 物联网 模块，主要实现 产品管理、设备管理、协议管理等功能。 |
...
| `develop-module-iot-api` | 业务模块 API / server / core 依赖。 |
...
| `spring-boot-starter-amqp` | 消息队列客户端能力。 |
...
# 运行该模块测试
mvn test -pl develop-module-iot/develop-module-iot-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-iot/develop-module-iot-server -am -Dmaven.test.skip=true
```
