...
会员中心模块，管理会员用户、等级、积分、签到、地址、标签、分组等用户侧能力。
...
| Maven Artifact | `develop-module-member` |
| Packaging | `pom` |
| Java 源文件数量 | 244 |
| 模块说明 | member 模块，我们放会员业务。 例如说：会员中心等等 |

## 子模块结构

| 子模块 | 职责 |
|---|---|
| `develop-module-member-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-member-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-member -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-member -am -Dmaven.test.skip=true
```
