...
erp 包下，企业资源管理（Enterprise Resource Planning）。 例如说：采购、销售、库存、财务、产品等等

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-erp/develop-module-erp-server` |
| Maven Artifact | `develop-module-erp-server` |
| Packaging | `jar` |
| Java 源文件数量 | 226 |
| 模块说明 | erp 包下，企业资源管理（Enterprise Resource Planning）。 例如说：采购、销售、库存、财务、产品等等 |
...
| `develop-module-erp-api` | 业务模块 API / server / core 依赖。 |
...
# 运行该模块测试
mvn test -pl develop-module-erp/develop-module-erp-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-erp/develop-module-erp-server -am -Dmaven.test.skip=true
```
