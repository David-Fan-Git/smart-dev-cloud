...
企业资源计划模块，提供采购、销售、库存、财务、产品、供应商、客户等企业经营能力。
...
| Maven Artifact | `develop-module-erp` |
| Packaging | `pom` |
| Java 源文件数量 | 234 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-erp-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-erp-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-erp -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-erp -am -Dmaven.test.skip=true
```
