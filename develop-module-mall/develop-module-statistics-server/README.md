...
statistics 模块，主要实现统计相关功能 例如：统计商品、会员、交易等功能。

## 基本信息

| 项目 | 内容 |
|---|---|
| 模块路径 | `develop-module-mall/develop-module-statistics-server` |
| Maven Artifact | `develop-module-statistics-server` |
| Packaging | `jar` |
| Java 源文件数量 | 87 |
| 模块说明 | statistics 模块，主要实现统计相关功能 例如：统计商品、会员、交易等功能。 |
...
| `develop-module-statistics-api` | 业务模块 API / server / core 依赖。 |
| `develop-module-promotion-api` | 业务模块 API / server / core 依赖。 |
| `develop-module-product-api` | 业务模块 API / server / core 依赖。 |
| `develop-module-trade-api` | 业务模块 API / server / core 依赖。 |
| `develop-module-member-api` | 业务模块 API / server / core 依赖。 |
| `develop-module-pay-api` | 业务模块 API / server / core 依赖。 |
...
# 运行该模块测试
mvn test -pl develop-module-mall/develop-module-statistics-server

# 打包该模块及其依赖
mvn clean package -pl develop-module-mall/develop-module-statistics-server -am -Dmaven.test.skip=true
```
