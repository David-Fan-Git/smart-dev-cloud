...
制造执行系统模块，提供生产计划、工单、工艺、工序、设备、质量、物料、报工等 MES 能力。
...
| Maven Artifact | `develop-module-mes` |
| Packaging | `pom` |
| Java 源文件数量 | 1139 |
...
| 子模块 | 职责 |
|---|---|
| `develop-module-mes-api` | 跨模块契约、DTO、枚举、CommonApi / RPC API。 |
| `develop-module-mes-server` | 业务实现、Controller、Service/ApplicationService、Domain、Infrastructure、DAL、MQ、Job。 |
...
```bash
# 编译该聚合模块及其子模块
mvn compile -pl develop-module-mes -am

# 打包该聚合模块及其子模块
mvn clean package -pl develop-module-mes -am -Dmaven.test.skip=true
```
